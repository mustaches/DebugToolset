// 乘法器（GPU 版，对应 isp_kernels.dart 的 multiplyMono）：两路 mono
// 打包纹理（w/2 × h，同尺寸由调用方校验）逐像素
// out = clamp((a+offset1)×(b+offset2)/maxValue, 0, maxValue)。
// 注：sampler 不作为函数参数传递（SkSL 转译不支持），两路采样内联展开。
...
void main() {
  ...
  float vA = floor(clamp(
      (ta.r + ta.g * 256.0 + uOff1) * (tb.r + tb.g * 256.0 + uOff2) /
          uMaxValue,
      0.0, uMaxValue) + 0.5);
  float vB = floor(clamp(
      (ta.b + ta.a * 256.0 + uOff1) * (tb.b + tb.a * 256.0 + uOff2) /
          uMaxValue,
      0.0, uMaxValue) + 0.5);
