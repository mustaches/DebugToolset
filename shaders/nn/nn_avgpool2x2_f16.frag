// NN AvgPool 2x2/s2/p0（fp16 打包路径，对应 GpuNnBackend.avgPool2x2Gpu）。
//
// 布局同 nn_conv3x3_f16.frag 头注释的折叠线性布局：逻辑纹素
// T=(gi*H+y)*W+x 装通道 4gi..4gi+3 的 4 个 half = 2 个物理纹素。输出
// 空间尺寸 floor(H/2)×floor(W/2)，通道数不变；奇数宽/高的末列/末行被
// 丢弃（torch AvgPool2d(k2,s2,p0) 语义：池化窗口恒为完整 2x2，除数恒 4）。
...
void main() {
  ...
  // 窗口恒完整（输出尺寸已按 floor(H/2) 截断），直接 4 tap 求均值。
  vec4 m = fetchIn4(gi, ix0, iy0) + fetchIn4(gi, ix0 + 1.0, iy0) +
           fetchIn4(gi, ix0, iy0 + 1.0) + fetchIn4(gi, ix0 + 1.0, iy0 + 1.0);
  m *= 0.25;
