...
(double, double) _fitWeibull(Float64List data) {
  ...
  profAdd('6a_weibull', swW.elapsedMicroseconds);
...
final profTimings = <String, int>{};
void profAdd(String name, int us) => profTimings[name] = (profTimings[name] ?? 0) + us;
...
Float64List _pinvSymmetric(Float64List a, int n) {
  ...
  for (var sweep = 0; sweep < 100; sweep++) {
    ...
    if (off == 0.0) break;
    final tresh = sweep < 4 ? 0.2 * off / (n * n) : 0.0;
    for (var p = 0; p < n; p++) {
      for (var r = p + 1; r < n; r++) {
        final apr = m[p * n + r];
...
double ilniqeScore(Uint8List rgba, int width, int height,
  ...
  profAdd('1_split_resize', swStage.elapsedMicroseconds); swStage.reset();
  w = h = normalizedWidth;
  if (debug != null) {
  ...
  for (var scale = 1; scale <= 2; scale++) {
    ...
    profAdd('2_grads_scale$scale', swStage.elapsedMicroseconds); swStage.reset();
    ...
    profAdd('3_loggabor_build', swStage.elapsedMicroseconds); swStage.reset();
    ...
    profAdd('4_fwd_fft', swStage.elapsedMicroseconds); swStage.reset();
    for (var f = 0; f < 12; f++) {
    ...
    profAdd('5_filter_loop', swStage.elapsedMicroseconds); swStage.reset();
    ...
    profAdd('6_block_features_scale$scale', swStage.elapsedMicroseconds); swStage.reset();
    if (scale == 1) {
      // 高斯平滑 + 隔点降采样（边界复制）。
  ...
  profAdd('7_downsample', swStage.elapsedMicroseconds); swStage.reset();
  // 两尺度特征按块拼接为 468 维。
  final blocks = nb * nb;
  ...
  profAdd('8a_pinv', swP.elapsedMicroseconds);
  var scoreSum = 0.0;
  for (final row in finalFeatures) {
  ...
  profAdd('8_pca_mvg', swStage.elapsedMicroseconds);
  profAdd('0_total', swAll.elapsedMicroseconds);
