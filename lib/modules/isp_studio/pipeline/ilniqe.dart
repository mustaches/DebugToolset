...
/// 马氏距离逐块平均（越大越差）。
///
/// 纯 Dart（无 Flutter 依赖），供后台 isolate 调用（计算量大，勿在
/// UI isolate 直接跑）；[ilniqeScoreParallel] 为多核并行版
/// （dart:isolate 临时 worker 池），与串行位级一致。
/// 模型参数见 ilniqe_model.dart。
library;
...
/// compute() 入口（多核并行版）：`{'rgba': Uint8List, 'width': int,
/// 'height': int, 'workers': int?}` → ILNIQE 分值（double）。
/// 结果与 [ilniqeScoreInIsolate] 位级一致（见 ilniqeScoreParallel）。
@pragma('vm:entry-point')
Future<double> ilniqeScoreParallelInIsolate(Map<String, Object?> msg) =>
    ilniqeScoreParallel(
        msg['rgba'] as Uint8List, msg['width'] as int, msg['height'] as int,
        workers: (msg['workers'] as num?)?.toInt() ?? 6);
...
/// ---------------------------------------------------------------------------
/// 单个 log-Gabor 滤波器响应：频域乘积 → 逆 FFT → 实/虚部的高斯
/// 导数梯度（[kx]/[ky] 为可分离核）与梯度幅度，返回 8 张 w×h 平面
/// [realRes, imagRes, prx, pry, pix, piy, realGm, imagGm]。
/// 顶层函数：并行路径（ilniqeScoreParallel）在 worker isolate 调用
/// 同一份代码，结果与串行逐滤波器计算位级一致。
List<Float64List> ilniqeFilterResponse(
    Float64List fftRe,
    Float64List fftIm,
    Float64List filt,
    int w,
  ...
  final realRes = fRe;
  final imagRes = fIm;
  final prx = _conv2dSep(realRes, w, h, kx, ky, replicate: false);
  final pry = _conv2dSep(realRes, w, h, ky, kx, replicate: false);
  final pix = _conv2dSep(imagRes, w, h, kx, ky, replicate: false);
  final piy = _conv2dSep(imagRes, w, h, ky, kx, replicate: false);
  ...
  return [realRes, imagRes, prx, pry, pix, piy, realGm, imagGm];
}
...
/// LDLᵀ 分解 + 多右端求解 A·X = B（A 对称正定 n×n；[b] 按行存
/// [nrhs] 个右端，返回同布局的解）。任一主元 ≤ 1e-15·迹（非正定/
/// 近奇异——pinv 的特征值截断会生效的情形）返回 null，调用侧回退
/// [_pinvSymmetric]。正定情况下与伪逆求解数学等价，但只需
/// O(n³/3)（Jacobi 特征分解为数十扫 O(n³)/扫，实测快约 30 倍）。
Float64List? _ldlSolve(Float64List a, int n, Float64List b, int nrhs) {
  var trace = 0.0;
  for (var i = 0; i < n; i++) {
    trace += a[i * n + i].abs();
  }
  final pivotEps = 1e-15 * trace;
  ...
  for (var i = 0; i < n; i++) {
    ...
    if (!(s > pivotEps)) return null;
    d[i] = s;
  }
  // 前代 L·z = b（L 对角恒 1），z ← D⁻¹·z，回代 Lᵀ·x = z。
  final x = Float64List.fromList(b);
  for (var r = 0; r < nrhs; r++) {
    final off = r * n;
    for (var i = 0; i < n; i++) {
      var s = x[off + i];
      for (var k = 0; k < i; k++) {
...
/// 块裁剪：复合特征图 [channels]（每张 w×h）的 [bx,by] 起 bs×bs
/// 区域，按通道序返回。并行路径在主 isolate 构建后发给 worker
/// （尺度 1 每块约 6MB，比整通道平面传输省约 35 倍）。
List<Float64List> ilniqeBlockCrops(
    List<Float64List> channels, int w, int bx, int by, int bs) {
  return [
...
/// 块特征（IceClear compute_feature 移植）：预裁剪块数据 [crops]
/// （按通道序，每块 bs×bs）→ 234 维。顶层函数：并行路径
/// （ilniqeScoreParallel）在 worker isolate 调用同一份代码，
/// 与串行位级一致。
List<double> ilniqeBlockFeaturesFromCrops(List<Float64List> crops, int bs) {
  Float64List crop(int ch) => crops[ch];
...
/// 串行路径的块特征：裁剪 + 计算（与并行路径共用同一实现）。
List<double> _ilniqeBlockFeatures(
        List<Float64List> channels, int w, int bx, int by, int bs) =>
    ilniqeBlockFeaturesFromCrops(
        ilniqeBlockCrops(channels, w, bx, by, bs), bs);
...
double ilniqeScore(Uint8List rgba, int width, int height,
  ...
  for (var scale = 1; scale <= 2; scale++) {
    ...
    for (var f = 0; f < 12; f++) {
      final resp =
          ilniqeFilterResponse(fftRe, fftIm, filters[f], w, h, kx, ky);
      if (scale == 1 && f == 0 && debug != null) {
        debug['logresp0'] = Float64List.fromList(resp[0]);
        debug['gm0'] = Float64List.fromList(resp[6]);
      }
  ...
  // 逐块马氏距离平均（块内 NaN 用均值替换）：q = diffᵀ·invCov·diff。
  // avgCov 正定时按 LDLᵀ 求解（与 pinv 等价，快约 30 倍）；近奇异
  // 时回退 Jacobi pinv（原路径）。
  final avgCov = Float64List(pcaDim * pcaDim);
  for (var i = 0; i < pcaDim * pcaDim; i++) {
    avgCov[i] = (ilniqeCovPris[i] + covDist[i]) / 2;
  }
  final nRows = finalFeatures.length;
  final diffs = Float64List(nRows * pcaDim);
  for (var r = 0; r < nRows; r++) {
    final row = finalFeatures[r];
    for (var j = 0; j < pcaDim; j++) {
      final v = row[j].isNaN ? muDist[j] : row[j];
      diffs[r * pcaDim + j] = v - ilniqeMuPris[j];
    }
  }
  final solved = _ldlSolve(avgCov, pcaDim, diffs, nRows);
  var scoreSum = 0.0;
  if (solved != null) {
    for (var r = 0; r < nRows; r++) {
      var q = 0.0;
      for (var j = 0; j < pcaDim; j++) {
        q += diffs[r * pcaDim + j] * solved[r * pcaDim + j];
      }
    ...
    for (var r = 0; r < nRows; r++) {
      ...
      for (var i = 0; i < pcaDim; i++) {
        var acc = 0.0;
        for (var j = 0; j < pcaDim; j++) {
          acc += invCov[i * pcaDim + j] * diffs[r * pcaDim + j];
        }
        q += diffs[r * pcaDim + i] * acc;
      }
      scoreSum += math.sqrt(q < 0 ? 0 : q);
    }
  }
  return scoreSum / nRows;
}

/// ILNIQE 质量分的多核并行版：每尺度的 log-Gabor 滤波器组（12 个）
/// 与分块特征（36 块）分发给本次调用自建的临时 worker 池并行计算，
/// worker 调用与串行版完全相同的函数（[ilniqeFilterResponse] /
/// [ilniqeBlockFeaturesFromCrops]），结果与 [ilniqeScore]
/// **位级一致**（滤波器/块之间无共享求和，各自独立产出）。
/// 用常驻池而非每任务 Isolate.run：JIT 模式下每个新 isolate 都要
/// 冷启动编译整个调用图（实测 36 次冷 spawn 比串行还慢一倍），
/// 池内 worker 只编译一次、四个阶段复用。流程与串行版逐段对应
/// （改动时请两边同步）。不支持 debug 中间量导出；
/// [workers] ≤ 1 时直接退化为串行版。
Future<double> ilniqeScoreParallel(Uint8List rgba, int width, int height,
    {int workers = 6}) async {
  if (workers <= 1) return ilniqeScore(rgba, width, height);
  if (width < 2 || height < 2 || rgba.length < width * height * 4) {
    return double.nan;
  }
  final pool = _IlniqeWorkerPool();
  await pool.start(workers);
  try {
    return await _ilniqeScoreParallelBody(rgba, width, height, pool, workers);
  } finally {
    pool.dispose();
  }
}

Future<double> _ilniqeScoreParallelBody(Uint8List rgba, int width,
    int height, _IlniqeWorkerPool pool, int workers) async {
  const sigmaForGauDerivative = 1.66;
  const kForLog = 0.00001;
  const normalizedWidth = 524;
  const minWaveLength = 2.4;
  ...
  for (var scale = 1; scale <= 2; scale++) {
    ...
    // log-Gabor 频域滤波（O3）：3 尺度 × 4 方向，12 个滤波器经
    // Isolate.run 并行（各自独立逆 FFT + 梯度，结果按序合并）。
    final filters = _logGaborFilters(
        w, h, minWaveLength / math.pow(scale, scaleFactorForLoG), sigmaOnf,
        mult, dThetaOnSigma);
    final fftRe = Float64List.fromList(o3);
    ...
    final responses = await Future.wait([
      for (var f = 0; f < 12; f++)
        pool.request('filter', [fftRe, fftIm, filters[f], w, h, kx, ky]),
    ]);
    for (final resp in responses) {
      channels.addAll((resp as List).cast<Float64List>());
    }
    // 重排为参考实现的通道顺序（参考按滤波器逐个交错追加：
    // logResponse 为 real0,imag0,real1,imag1,...，partialDer 与 GM 同理）。
    final ordered = <Float64List>[
    ...
    // 分块特征（块边长 84/scale）：主 isolate 构建各块裁剪（memcpy
    // 级），按块分给 workers 路 Isolate.run 并行（块间独立，按序
    // 合并，与串行位级一致）。
    final bs = _kIlniqeBlock ~/ scale;
    final blockJobs = <List<Float64List>>[
      for (var by = 0; by < nb; by++)
        for (var bx = 0; bx < nb; bx++)
          ilniqeBlockCrops(ordered, w, bx * bs, by * bs, bs),
    ];
    final nJobs = blockJobs.length;
    final per = (nJobs + workers - 1) ~/ workers;
    final featChunks = await Future.wait([
      for (var s = 0; s < nJobs; s += per)
        pool.request('blocks', [
          blockJobs.sublist(s, s + per > nJobs ? nJobs : s + per),
          bs,
        ]),
    ]);
    for (final chunk in featChunks) {
      allFeatures.addAll((chunk as List).cast<List<double>>());
    }
...
/// ---------------------------------------------------------------------------
/// 并行版临时 worker 池（ilniqeScoreParallel 每次调用自建自毁）
/// ---------------------------------------------------------------------------

/// worker 入口：逐条处理 `[id, kind, ...payload]`，回 `[id, 结果]`，
/// 失败回 `[id, 'error', 消息]`。
/// - 'filter'：payload = [fftRe, fftIm, filt, w, h, kx, ky]
///   → `List<Float64List>`（8 张响应平面）。
/// - 'blocks'：payload = [块裁剪列表 `List<List<Float64List>>`, bs]
///   → `List<List<double>>`（各块 234 维特征）。
@pragma('vm:entry-point')
void _ilniqePoolWorkerMain(SendPort ui) {
  ...
  port.listen((msg) {
    ...
    try {
      if (req[1] == 'filter') {
        ui.send([
          id,
          ilniqeFilterResponse(
              req[2] as Float64List,
              req[3] as Float64List,
              req[4] as Float64List,
              req[5] as int,
              req[6] as int,
              req[7] as Float64List,
              req[8] as Float64List),
        ]);
      } else {
        final crops = (req[2] as List).cast<List<Float64List>>();
        final bs = req[3] as int;
        ui.send([
          id,
          [for (final c in crops) ilniqeBlockFeaturesFromCrops(c, bs)],
        ]);
      }
    } catch (e) {
...
/// 单次 ILNIQE 评分自用的临时 isolate 池：spawn 一次，滤波器/分块
/// 四个阶段复用（避免每任务冷 spawn 的 JIT 重复编译，见
/// ilniqeScoreParallel 注释），用完即毁。请求轮转分配到各 worker。
class _IlniqeWorkerPool {
    ...
    for (var i = 0; i < n; i++) {
      ...
      _subs.add(port.listen((msg) {
        ...
        if (resp.length > 2 && resp[1] == 'error') {
          c.completeError(StateError('ILNIQE worker 失败: ${resp[2]}'));
        } else {
          c.complete(resp[1]);
        }
      }));
      _isolates
          .add(await Isolate.spawn(_ilniqePoolWorkerMain, port.sendPort));
      readies.add(ready.future);
    }
    _send.addAll(await Future.wait(readies));
  }

  /// 发送一个任务，回结果对象（'filter' → `List`，'blocks' → `List`）。
  Future<Object?> request(String kind, List<Object?> payload) {
    final id = _reqId++;
    final c = Completer<Object?>();
    _pending[id] = c;
    _send[_rr % _send.length].send([id, kind, ...payload]);
    _rr++;
    return c.future;
  }
  ...
  void dispose() {
    ...
    for (final c in _pending.values) {
      c.completeError(StateError('ILNIQE worker 池已终止'));
    }
    _pending.clear();
  }
}
