...
- 打包方式：`unpacked_lsb`（默认）/ `unpacked_msb` / `mipi`；unpacked 两种方式**固定每像素 2 字节**（16 位字，与位深无关），`mipi` 为 CSI-2 紧凑打包（仅 10/12bit）；
- 小端序：开关，默认开；
...
- **预览**：5 输入（RGB/YUV/HSL/Mono/RAW，互斥）+ 4 输出透传（可串接/桥接）。RAW 输入直接接 Bayer 马赛克帧：不做去马赛克，每个像素的值作为亮度显示灰度图（链中无 Gamma 节点时按 gamma 2.2 映射）。参数：**播放帧率** `fps`（默认 30，1~60；视频源打开文件时自动填为原生帧率）、**预览帧数** `frameCount`（默认 30，1~100000；只取源前 N 帧）。节点内嵌屏幕 + 播放控制条（见下文）；
...
- **视频输入互斥组**：预览/仪器/输出节点的 RGB/YUV/HSL/Mono（预览节点另有 RAW）输入端口同时只能接一路，其余置灰；RAW 域算子的 Bayer/Mono 输入同理。
